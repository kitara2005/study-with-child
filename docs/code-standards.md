...
See `codebase-summary.md` for full directory tree.

**Key patterns:**

- `app/admin/` - Admin routes with role protection in layout.tsx
- `app/(auth)/` - Auth pages (signup, login, callback)
- `app/mon/[subject]/[chapter]/[lesson]/` - Content pages with ISR
- `app/api/admin/` - Admin CRUD endpoints
- `app/api/exercises/submit/` - Exercise grading
- `content/` - JSON lesson content, loaded on startup
- `lib/supabase/` - Both server & client Supabase instances
...
### Form Patterns (Zod validation only in MVP)
...
export function ExerciseForm({ lesson }: ExerciseFormProps) {
  const [answers, setAnswers] = useState<Record<string, string>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = exerciseSchema.parse(data); // Validate first
...
**Note:** React Hook Form deferred post-MVP (KISS principle).
